...
os.system(
    "cd file_system_backend && find . -type f -name '*' -mmin +15 -exec rm -v {} \; && cd .."
)
...
for xi in range(0, 6):
    for yi in range(0, 7):
        ...
        chips.append(
            html.Div(
                className="chipstuete",
                children=html.Div("", id=str(xi) + str(yi), className="chips grau"),
            )
        )
...
for yi in range(0, 7):
    ...
    buttons.append(
        html.Div(
            className="buttontuete",
            children=html.Button(
                "", id=bid, className="mybutton button-primary", n_clicks=0
            ),
        )
    )
...
dashapp.layout = html.Div(
    className="container",
    style={"max-width": "1200px", "margin-top": "15px"},
    children=[
        html.Div(
            className="row",
            children=[
                html.Div(className="seven columns", id="buttonrow", children=buttons),
                html.Div(
        ...
        html.Div(
            className="row",
            children=[
                html.Div(className="seven columns", id="board", children=chips),
                html.Div(
                    id="command",
                    className="three columns",
                    children=[
                        html.Div(
                            children="Tap buttons on the top to play",
                            className="explain",
                        ),
                        ...
                        html.Div(
                            "search depth of computer enemy",
                            className="explain",
                            style={"margin-top": "10px", "margin-bottom": "5px"},
                        ),
                        dcc.Dropdown([2, 3, 4, 5], 2, id="kselect"),
                        dcc.Textarea(
                            id="textarea",
...
def udpateboard(mode, b0, b1, b2, b3, b4, b5, b6, nst, k, cf):
    ...
    return (*cf.converttoouputlist(), *cf.turntostyle(), Serverside(cf))
